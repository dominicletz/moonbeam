...
use rocksdb::{
	BlockBasedOptions, ColumnFamily, ColumnFamilyDescriptor, CompactOptions, DBCompressionType, Options,
...
impl CompactionProfile {
	...
	pub fn ssd() -> CompactionProfile {
		CompactionProfile { initial_file_size: 64 * MB as u64, block_size: 256 * KB }
	}
...
"#;
	...
	fn rocksdb_settings() {
		...
		let zstd_compression = settings.matches("Options.compression: Zstd").collect::<Vec<_>>().len();
		// All columns use Zstd
		assert_eq!(zstd_compression, NUM_COLS + 1);
		let zstd_bottommost = settings
			.matches("Options.bottommost_compression: Zstd")
			.collect::<Vec<_>>()
			.len();
		assert_eq!(zstd_bottommost, NUM_COLS + 1);
